...
class Auth extends Base
{
	...
	{
		if ($this->session->userdata('user_role') == 'customer' && is_foodtruck_owner($this->session->userdata('user_id'))) {
			$this->session->set_userdata('user_role_id', 3);
			$this->session->set_userdata('user_role', "owner");
			$this->session->set_userdata('owner_login', 1);
			$this->session->set_flashdata('flash_message', get_phrase('successfully_switched_to_foodtruck_owner', true));
		} elseif ($this->session->userdata('user_role') == 'owner' && is_foodtruck_owner($this->session->userdata('user_id'))) {
