...
class Report extends Authorization
{
    ...
    function index()
    {
        ...
        $page_data['foodtruck_id'] = (isset($_GET['foodtruck_id']) && $_GET['foodtruck_id'] != "all") ? sanitize($_GET['foodtruck_id']) : "all";
    ...
    function details($foodtruck_id = null)
    {
        if (empty($foodtruck_id)) {
            error(get_phrase('select_a_foodtruck'), site_url('report'));
        }
        $foodtruck_details = $this->foodtruck_model->get_by_id($foodtruck_id);
        $page_data['foodtruck_details'] = $foodtruck_details;
        $page_data['page_name'] = 'report/index';
        $page_data['report_type'] = 'details';
        $page_data['page_title'] = get_phrase("commission_details_for_") . ' ' . $foodtruck_details['name'];
        $page_data['commission_details'] = $this->report_model->get_commission_details($foodtruck_id);
        $this->load->view('backend/index', $page_data);
    }
    ...
    function pay()
    {
        ...
        $response = $this->report_model->pay_to_foodtruck_owner();
        if ($response) {
