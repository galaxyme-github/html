<script type="text/javascript">
  ...
  function showModalWithHeader(url, header, thumbnail) {
    let modalHeader = '#modal-with-header';

    if (thumbnail != "placeholder.png") {
      modalHeader = '#thumbnail-modal';
    }

    // SHOWING AJAX PRELOADER IMAGE
    jQuery(`${modalHeader} .modal-body`).html('...');
    // LOADING THE AJAX MODAL
    jQuery(modalHeader).modal('show', {
      backdrop: 'true'
    });
    ...
    $.ajax({
      ...
      success: function(response) {
        if (thumbnail != "placeholder.png") {
          jQuery(`${modalHeader} .modal-content`).html(response);
        } else {
          jQuery(`${modalHeader} .title`).html(header);
          jQuery(`${modalHeader} .modal-body`).html(response);
        }
      }
    });
  }
...
<!--THUMBNAIL MODAL WITH AVATAR-->
<div class="modal fade" id="thumbnail-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
