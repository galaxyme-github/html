...
<div class="modal-body text-center mb-1">
    ...
    <?php if ($menu_details['servings'] == "menu") : ?>
        ...
        <?php if (is_open($menu_details['foodtruck_id'])) : ?>
            <?php if ($menu_details['availability']) : ?>
                <button class="btn btn-success btn-block" onclick="addToCart('<?php echo sanitize($menu_details['id']); ?>', 'menu', $('#quantity_for_menu').val())"><?php echo site_phrase('add_to_cart'); ?></button>
            <?php else : ?>
    ...
    <?php else : ?>
        ...
        <small><?php echo site_phrase('full_plate_price', true); ?> : <strong><?php echo currency(sanitize(get_menu_price($menu_details['id'], "full_plate"))); ?></strong></small><br>
        <small><?php echo site_phrase('half_plate_price', true); ?> : <strong><?php echo currency(sanitize(get_menu_price($menu_details['id'], "half_plate"))); ?></strong></small>

        <div class="form-group">
            <select class="custom-select mb-2 mr-sm-2 mb-sm-0 form-control plate-servings" id="plate-servings">
                <option value="full_plate"><?php echo site_phrase('full_plate'); ?></option>
                <option value="half_plate"><?php echo site_phrase('half_plate'); ?></option>
            </select>
        </div>
        <div class="form-group">
        ...
        <?php if (is_open($menu_details['foodtruck_id'])) : ?>
            <?php if ($menu_details['availability']) : ?>
                <button class="btn btn-success btn-block" onclick="addToCart('<?php echo sanitize($menu_details['id']); ?>', $('#plate-servings').val(), $('#quantity_for_full_plate').val())"><?php echo site_phrase('add_to_cart'); ?></button>
            <?php else : ?>
                <button class="btn btn-secondary btn-block"><?php echo site_phrase('unavailable_item', true); ?></button>
            <?php endif; ?>
        <?php else : ?>
