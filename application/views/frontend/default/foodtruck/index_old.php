...
<section class="reserve-block">
  <div class="container">
    <div class="row">
      <div class="col-md-8">
        <h5 class="mb-3">
          <?php echo sanitize($foodtruck_details['name']); ?>
          <div class="reserve-rating">
            <span><?php echo sanitize($foodtruck_details['rating']); ?></span>
          </div>
        </h5>
        <div class="reserve-description">
          <?php foreach (json_decode($foodtruck_details['cuisine']) as $cuisine) : ?>
            <?php
            ...
            <?php endif; ?>
          <?php endforeach; ?>
          <?php if (count(json_decode($foodtruck_details['cuisine'])) == 0) : ?>
      ...
      <div class="col-md-4">
        <div class="d-block">
          <p class="reserve-description text-dark d-block text-right"><?php echo site_phrase('delivery_charge'); ?> : <strong><?php echo delivery_charge($foodtruck_details['id']) > 0 ? currency(sanitize(delivery_charge($foodtruck_details['id']))) : site_phrase('free'); ?></strong>.</p>
          <p class="reserve-description text-dark d-block text-right"><?php echo site_phrase('maximum_time_to_deliver'); ?> : <strong><?php echo sanitize(maximum_time_to_deliver($foodtruck_details['id'])); ?></strong> <?php echo site_phrase('minutes'); ?>.</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-9 responsive-wrap">
                ...
                foreach ($menus as $key => $menu) : ?>
                  <div class="col-xl-3 col-lg-4 col-md-6 featured-responsive">
                    <div class="featured-place-wrap food-item-card">
                      <a href="javascript:void(0)" onclick="showModalWithHeader('<?php echo site_url('modal/showup/foodtruck/menu/' . $menu['id']); ?>', '<?php echo sanitize($menu['name']); ?>')">
                        ...
                        <div class="bottom-icons <?php if ($menu['servings'] == "menu") echo 'mt-22'; ?>">
                          <?php if ($menu['availability']) : ?>
                            <div class="closed-now">
                              <a href="javascript:void(0)" onclick="showCartModal('<?php echo site_url('modal/showup/foodtruck/cart/' . $menu['id']); ?>', '<?php echo site_phrase('add_to_cart'); ?>')">
        ...
        <!-- REVIEW PORTION -->
        <?php $reviews = $this->review_model->get_by_condition(['foodtruck_id' => $foodtruck_details['id']]); ?>
      ...
      <div class="col-md-3 responsive-wrap">
        <div class="contact-info">
          ...
          <div class="address">
            ...
            <p><?php echo getter(sanitize($foodtruck_details['address']), site_phrase('not_found')); ?></p>
          </div>
          <div class="address">
            <span class="icon-screen-smartphone"></span>
            <p><?php echo getter(sanitize($foodtruck_details['phone']), site_phrase('not_found')); ?></p>
          </div>
          <div class="address">
            <span class="icon-link"></span>
            <p><a href="<?php echo sanitize($foodtruck_details['website']); ?>" target="_blank" class="text-body"><?php echo getter(sanitize($foodtruck_details['website']), site_phrase('not_found')); ?></a></p>
          </div>
          <div class="address pb-3">
            <span class="icon-clock"></span>
            <p>
              <?php if (!empty($foodtruck_details['schedule'])) : ?>
                <?php $time_configurations = json_decode($foodtruck_details['schedule'], true);
