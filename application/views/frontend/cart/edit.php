...
<div class="modal-body text-center mb-1">
    ...
    <?php if ($menu_details['servings'] == "menu") : ?>
        ...
        <button class="btn btn-success btn-block" onclick="updateCart('<?php echo sanitize($cart_item_details['id']); ?>', '<?php echo sanitize($menu_details['id']); ?>', 'menu', $('#quantity_for_menu').val())"><?php echo site_phrase('add_to_cart'); ?></button>
    ...
    <?php else : ?>
        ...
        <button class="btn btn-success btn-block" onclick="updateCart('<?php echo sanitize($cart_item_details['id']); ?>', '<?php echo sanitize($menu_details['id']); ?>', $('#plate-servings').val(), $('#quantity_for_full_plate').val())"><?php echo site_phrase('add_to_cart'); ?></button>
