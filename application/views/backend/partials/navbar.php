<nav class="main-header navbar navbar-expand navbar-white navbar-light">
  ...
  <ul class="navbar-nav">
    ...
    <li class="nav-item mt-1 d-sm-inline-block">
      <a href="<?php echo site_url("site"); ?>" class="btn btn-sm btn-primary" role="button"><i class="far fa-paper-plane"></i> <?php echo get_phrase('view_website'); ?></a>
    </li>
  ...
  <ul class="navbar-nav ml-auto">
    <?php if ($this->session->userdata('user_role') == "customer" || $this->session->userdata('user_role') == "owner") : ?>
      <?php if (is_foodtruck_owner($this->session->userdata('user_id'))) : ?>
        <!--li class="nav-item mt-1 d-sm-inline-block mr-1">
          <?php if ($this->session->userdata('user_role') == "customer") : ?>
            <a href="<?php echo site_url('auth/switch_role'); ?>" class="btn btn-sm btn-success bg-gradient-olive role-switcher" role="button" data-toggle="tooltip" data-placement="Bottom" title="<?php echo get_phrase('a_customer_can_be_a_foodtruck_owner'); ?>">
              <i class="fas fa-user-tie"></i> <?php echo get_phrase('switch_to_foodtruck_owner', true); ?>
            </a>
          <?php elseif ($this->session->userdata('user_role') == "owner") : ?>
            <a href="<?php echo site_url('auth/switch_role'); ?>" class="btn btn-sm btn-success bg-gradient-olive role-switcher" role="button" data-toggle="tooltip" data-placement="Bottom" title="<?php echo get_phrase('a_foodtruck_owner_can_also_have_all_the_facilities_of_a_customer'); ?>">
      ...
      <li class="nav-item dropdown mr-3">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <?php if ($this->session->userdata('user_role') == 'admin') : ?>
            <div class="dropdown-divider"></div>
            <a href="<?php echo site_url('foodtruck/pending'); ?>" class="dropdown-item">
