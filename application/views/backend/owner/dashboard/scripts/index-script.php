...
$owned_foodtruck_ids = $this->foodtruck_model->get_approved_foodtruck_ids_by_owner_id($this->session->userdata('user_id'));
$order_codes = count($owned_foodtruck_ids) ? $this->order_model->get_order_code_by_foodtruck_id($owned_foodtruck_ids) : array();
...
for ($i = 0; $i < 12; $i++) {
    ...
    if (count($owned_foodtruck_ids)) {
        if (count($order_codes)) {
            ...
            $this->db->where_in('foodtruck_id', $owned_foodtruck_ids);
            $total_foodtruck_owner_avenue = $this->db->get('commission_details')->row()->owner_commission;
        } else {
            $total_foodtruck_owner_avenue = 0;
        }
    } else {
        $total_foodtruck_owner_avenue = 0;
    }
    $total_foodtruck_owner_avenue > 0 ? array_push($month_wise_income, $total_foodtruck_owner_avenue) : array_push($month_wise_income, 0);
}
// EACH DAY ORDER DATA
$each_day_orders = [];
for ($i = 1; $i <= date("t"); $i++) {
    if (count($owned_foodtruck_ids)) {
        $order_codes = $this->order_model->get_order_code_by_foodtruck_id($owned_foodtruck_ids);
        if (count($order_codes)) {
            $conditions = [
...
$number_of_approved_foodtrucks = count($owned_foodtruck_ids) ? count($this->foodtruck_model->get_approved_foodtruck_ids_by_owner_id($this->session->userdata('user_id'))) : 0;
$number_of_pending_foodtrucks  = count($owned_foodtruck_ids) ? count($this->foodtruck_model->get_pending_foodtruck_ids_by_owner_id($this->session->userdata('user_id'))) : 0;
?>
...
<script>
    ...
    am4core.ready(function() {
        ...
        var chart = am4core.create("foodtruck-status", am4charts.PieChart);

        // Set data
        var selected;
        var types = [{
            type: '<?php echo get_phrase('approved_foodtruck'); ?>',
            percent: '<?php echo sanitize($number_of_approved_foodtrucks); ?>',
            color: am4core.color("#4CAF50")
        }, {
            type: '<?php echo get_phrase('requested_foodtruck'); ?>',
            percent: '<?php echo sanitize($number_of_pending_foodtrucks); ?>',
