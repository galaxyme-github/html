    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-3">
                ...
                <?php if (is_foodtruck_owner($customer['id'])) : ?>
                    ...
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            <?php
                            $foodtrucks = $this->foodtruck_model->get_foodtrucks_by_condition(['owner_id' => sanitize($customer['id'])]); ?>
                            <?php
                            if (count($foodtrucks) > 0) :
                                foreach ($foodtrucks as $key => $foodtruck) : ?>
                                    <strong><i class="fas fa-utensils"></i></strong> <a href="<?php echo site_url('home/foodtruck/' . rawurlencode(sanitize($foodtruck['slug'])) . '/' . sanitize($foodtruck['id'])); ?>" target="_blank"><span class="text-muted"><?php echo sanitize($foodtruck['name']); ?></span></a>
                                    <hr>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <div class="alert alert-warning lighten-warning"><?php echo get_phrase('this_foodtruck_owner_does_not_have_any_foodtruck_created'); ?>.</div>
                            <?php endif; ?>
                        </div>
                        <!-- /.card-body -->
                    </div>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($orders as $order) : ?>
                                            <tr>
                                                <td>
                                                    <?php
                                                    $foodtruck_ids = $this->order_model->get_foodtruck_ids($order['code']);
                                                    foreach ($foodtruck_ids as $foodtruck_id) :
                                                        $foodtruck_detail = $this->foodtruck_model->get_by_id($foodtruck_id); ?>
                                                        <a href="<?php echo site_url('home/foodtruck/' . rawurlencode(sanitize($foodtruck_detail['slug'])) . '/' . sanitize($foodtruck_detail['id'])); ?>" class="text-dark" target="_blank"><small class="d-block"> ∙ <?php echo sanitize($foodtruck_detail['name']); ?></small></a>
                                                    <?php endforeach; ?>
                                                </td>
                                                <td>
