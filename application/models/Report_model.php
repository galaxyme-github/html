...
class Report_model extends MY_Model
{
    ...
    public function get_commission_details_as_admin($foodtruck_id)
    {
        $this->db->order_by("id", "desc");
        return $this->db->get_where('commission_details', ['foodtruck_id' => $foodtruck_id])->result_array();
    }

    public function get_commission_details_as_owner($foodtruck_id)
    {
        $this->db->order_by("id", "desc");
        return $this->db->get_where('commission_details', ['foodtruck_id' => $foodtruck_id])->result_array();
    }
    ...
    {
        $conditions['foodtruck_id'] = (isset($_GET['foodtruck_id']) && $_GET['foodtruck_id'] != "all") ? sanitize($_GET['foodtruck_id']) : null;
     ...
     *
     * @param [INT] $foodtruck_id
     * @return INT
     */
    public function get_total_payable_commission($foodtruck_id)
    {
        $total_commission = 0;
     ...
     * @return INT
     */
    public function get_total_paid_commission($foodtruck_id)
    {
        $this->db->where('foodtruck_id', $foodtruck_id);
    ...
    public function devide_commission($order_code)
    {
        ...
        foreach ($query as $key => $row) {
            $foodtruck_details = $this->foodtruck_model->get_by_id($row['foodtruck_id']);

            /* GET TOTAL MENU PRICE */
            $this->db->select_sum('total');
            $total_menu_bill = $this->db->get_where('order_details', ['order_code' => $order_code, 'foodtruck_id' => $row['foodtruck_id']])->row()->total;

            /* MENU PRICE WITH DELIVERY CHARGE */
            $sub_total = $total_menu_bill + delivery_charge($row['foodtruck_id']);
            ...
            if ($foodtruck_details['owner_id'] == $admin_details['id']) {
                $report_data['admin_commission'] = $grand_total;
                $report_data['owner_commission'] = 0;
            } else {
                $foodtruck_revenue_percentage = get_delivery_settings('foodtruck_revenue');
                $foodtruck_revenue = round(($grand_total * $foodtruck_revenue_percentage) / 100);
                $report_data['admin_commission'] = $grand_total - $foodtruck_revenue;
                $report_data['owner_commission'] = $foodtruck_revenue;
            }

            $report_data['foodtruck_id'] = $row['foodtruck_id'];
            ...
            $paid_commission_previous_data_checker = $this->db->get_where('paid_commissions', ['foodtruck_id' => $row['foodtruck_id']]);
            if ($paid_commission_previous_data_checker->num_rows() == 0) {
                $this->db->insert('paid_commissions', ['foodtruck_id' => $row['foodtruck_id']]);
            }
        }
    }

    public function pay_to_foodtruck_owner()
    {
        $foodtruck_id = required(sanitize($this->input->post('foodtruck_id')));
        $amount_to_pay = required(sanitize($this->input->post('amount_to_pay')));
        $foodtruck_details = $this->db->get_where('foodtrucks', ['id' => $foodtruck_id, 'status' => 1]);
        if ($foodtruck_details->num_rows() > 0) {
            $total_paid_commission = $this->get_total_paid_commission($foodtruck_id);
            $total_payable_commission = $this->get_total_payable_commission($foodtruck_id);
            ...
            if (!$amount_to_pay || $amount_to_pay > $due_amount) {
                ...
                $paid_commission_previous_data = $this->db->get_where('paid_commissions', ['foodtruck_id' => $foodtruck_id])->row_array();
            ...
            error(get_phrase('invalid_foodtruck_id'), site_url('report'));
        }
    }
}
