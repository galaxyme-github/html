...
class Settings_model extends MY_Model
{
    ...
    public function update_website_settings()
    {
        ...
        $social_link['pinterest'] = sanitize($this->input->post('pinterest_link'));
    ...
    public function update_revenue_settings()
    {
        ...
        if ($admin_revenue >= 0 && $admin_revenue <= 100) {
            $foodtruck_revenue = 100 - $admin_revenue;

            $this->db->where('key', 'admin_revenue');
            $this->db->update('delivery_settings', ['value' => $admin_revenue]);

            $this->db->where('key', 'foodtruck_revenue');
            $this->db->update('delivery_settings', ['value' => $foodtruck_revenue]);
            return true;
        } else {
